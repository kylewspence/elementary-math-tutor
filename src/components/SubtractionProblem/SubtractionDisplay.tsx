...
interface SubtractionDisplayProps {
    ...
    onAnswerClear: (columnPosition: number, fieldType: 'difference' | 'borrow' | 'adjusted') => void;
    ...
    onFieldClick: (columnPosition: number, fieldType: 'difference' | 'borrow' | 'adjusted') => void;
...
const SubtractionDisplay: React.FC<SubtractionDisplayProps> = ({
    ...
    // Helper function to check if a column needs borrowing
    const needsBorrowing = useCallback((columnPosition: number) => {
        if (!problem) return false;
        const step = problem.steps.find(s => s.columnPosition === columnPosition);
        return step !== undefined && step.needsBorrow;
    }, [problem]);

    // Helper function to check if a column was borrowed from (adjusted minuend)
    const wasBorrowedFrom = useCallback((columnPosition: number) => {
        if (!problem) return false;
        const step = problem.steps.find(s => s.columnPosition === columnPosition);
        return step !== undefined && step.lentTo > 0;
    }, [problem]);
    ...
    const getAllRequiredFields = useCallback(() => {
        ...
        // Process each column - UI renders borrow boxes first, then adjusted minuend, then difference
        for (const step of orderedSteps) {
            // Add borrow field if this column needs borrowing
            if (needsBorrowing(step.columnPosition)) {
                fields.push({
            ...
            // Add adjusted minuend field if this column was borrowed from
            if (wasBorrowedFrom(step.columnPosition)) {
                fields.push({
                    columnPosition: step.columnPosition,
                    fieldType: 'adjusted'
                });
        ...
        return fields;
    }, [problem, needsBorrowing, wasBorrowedFrom]);

    // Helper to get user's answer for a specific field
    const getUserAnswer = useCallback((columnPosition: number, fieldType: 'difference' | 'borrow' | 'adjusted'): SubtractionUserAnswer | undefined => {
    ...
    const getInputVariant = useCallback((columnPosition: number, fieldType: 'difference' | 'borrow' | 'adjusted') => {
    ...
    const handleInputChange = useCallback((columnPosition: number, fieldType: 'difference' | 'borrow' | 'adjusted', value: string) => {
        if (value === '') {
    ...
    const createInput = useCallback((columnPosition: number, fieldType: 'difference' | 'borrow' | 'adjusted') => {
        return (
            <Input
    ...
    return (
        <div className="subtraction-display bg-white p-8 rounded-xl border-2 border-gray-200 font-mono pb-32">
            ...
            {/* Borrowing help legend - only show if problem has borrowing */}
            {problem && problem.steps.some(step => step.needsBorrow) && !isSubmitted && (
                <div className="text-center mb-4">
                    <div className="inline-block bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 text-blue-800 text-sm">
                        <div className="font-semibold mb-1">💡 Borrowing Help:</div>
                        <div className="flex items-center justify-center gap-4 text-xs">
                            <span className="flex items-center gap-1">
                                <span className="bg-yellow-100 px-2 py-1 rounded border border-yellow-300">Yellow</span>
                                = Borrow amount
                            </span>
                            <span className="flex items-center gap-1">
                                <span className="bg-blue-100 px-2 py-1 rounded border border-blue-200">Blue</span>
                                = New digit after borrowing
                            </span>
                            <span className="flex items-center gap-1">
                ...
                <div className="flex justify-center">
                    <div className="subtraction-workspace relative">
                        ...
                        <div className="subtraction-grid relative">
                            {/* Borrow row - above the minuend with enhanced visualization */}
                            <div className="flex justify-end mb-1">
                                {/* Borrow boxes for each column that needs borrowing */}
                                {displaySteps.map((step) => (
                                    ...
                                    >
                                        {needsBorrowing(step.columnPosition) && (
                                            <>
                                                {/* Borrow input with visual enhancement */}
                                                <div className="scale-70 transform origin-center">
                                                    <div className="bg-yellow-50 rounded-lg p-1 border border-yellow-300">
                                                        {createInput(step.columnPosition, 'borrow')}
                                                    </div>
                                                </div>

                                                {/* Borrowing indicator arrow pointing down */}
                                                <div className="absolute -bottom-1 text-yellow-600 text-xs font-bold">
                                                    ↓
                                                </div>
                                            </>
                                        )}
                            ...
                            {/* Minuend row with adjusted values and borrowing visualization */}
                            <div className="flex justify-end mb-2">
                                {displaySteps.map((step) => (
                                    <div
                                        key={`minuend-${step.columnPosition}`}
                                        className="flex flex-col items-center justify-center text-xl relative"
                                        style={{ width: `${BOX_TOTAL_WIDTH}px`, height: `${ROW_HEIGHT}px` }}
                                    >
                                        {/* Adjusted minuend input (if borrowed from) - positioned above */}
                                        {wasBorrowedFrom(step.columnPosition) && (
                                            <div className="absolute -top-3 scale-75 transform origin-center">
                                                <div className="bg-blue-50 rounded-lg p-1 border border-blue-200">
                                                    {createInput(step.columnPosition, 'adjusted')}
                                                </div>
                                            </div>
                                        )}

                                        {/* Borrowing arrow visualization */}
                                        {wasBorrowedFrom(step.columnPosition) && (
                                            <div className="absolute -top-1 -right-2 text-blue-500 text-sm font-bold">
                                                ↘
                                            </div>
                                        )}

                                        {/* Original minuend digit with visual indication of borrowing */}
                                        <div className={`relative ${wasBorrowedFrom(step.columnPosition)
                                            ? 'text-gray-400 line-through'
                                            : needsBorrowing(step.columnPosition)
                                                ? 'text-red-600 font-bold'
                                                : ''
                                            }`}>
                                            {step.minuendDigit === 0 && step.columnPosition > 0 ? '' : step.minuendDigit}

                                            {/* Small indicator for columns that need borrowing */}
                                            {needsBorrowing(step.columnPosition) && !wasBorrowedFrom(step.columnPosition) && (
                            ...
                            <div className="flex justify-end mb-2">
                                <div className="flex items-center justify-center text-xl font-bold mr-2">
                                    −
                                </div>
                                {displaySteps.map((step) => (
                                    <div
                                        ...
                                        style={{ width: `${BOX_TOTAL_WIDTH}px`, height: `${ROW_HEIGHT}px` }}
                                    >
                                        {step.subtrahendDigit === 0 && step.columnPosition > 0 ? '' : step.subtrahendDigit}
                                    </div>
                                ))}
                            </div>

                            {/* Line to separate subtrahend from difference */}
                            <div className="border-b-2 border-gray-800 w-full mb-2"></div>

                            {/* Difference row (user inputs) with borrowing context */}
                            <div className="flex justify-end">
                                ...
                                {displaySteps.map((step) => (
                                    <div
                                        ...
                                        style={{ width: `${BOX_TOTAL_WIDTH}px`, height: `${ROW_HEIGHT}px` }}
                                    >
                                        {/* Difference input with subtle highlighting for borrowing columns */}
                                        <div className={`${needsBorrowing(step.columnPosition) ? 'bg-yellow-25 rounded-lg p-1' : ''}`}>
                                            {createInput(step.columnPosition, 'difference')}
                                        </div>

                                        {/* Small indicator showing the borrowing relationship */}
                                        {needsBorrowing(step.columnPosition) && (
                                            <div className="absolute -top-2 -left-1 text-yellow-600 text-xs">
                                                ↗
                                            </div>
                                        )}
                                    </div>
