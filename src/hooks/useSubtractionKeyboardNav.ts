...
export function useSubtractionKeyboardNav(problem: SubtractionProblem | null, userAnswers: SubtractionUserAnswer[] = [], isSubmitted: boolean = false) {
    ...
    // Helper function to check if a column needs borrowing - matches UI logic exactly
    ...
    // Helper function to check if a column was borrowed from (lent to previous column) - matches UI logic exactly
    ...
    const allFields = useMemo(() => {
        ...
        for (const step of orderedSteps) {
            // Add borrow field if this column needs borrowing (matches UI conditional rendering)
            ...
            // Add adjusted minuend field if this column was borrowed from (matches UI conditional rendering)
    ...
    const jumpToField = useCallback((columnPosition: number, fieldType: 'difference' | 'borrow' | 'adjusted') => {
        setCurrentFocus({
            columnPosition,
            fieldType
        });
